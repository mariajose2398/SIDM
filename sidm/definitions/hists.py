...
hist_defs = {
    ...
    "genE_pt": h.Histogram(
        [
            h.Axis(hist.axis.Regular(100, 0, 200, name=r"gen $e$ $p_T$ $(GeV)$"),
                   lambda objs, mask: abs(objs["genEs"].pt)),
        ],
    ),
    "genE_pt_highRange": h.Histogram(
        [
            h.Axis(hist.axis.Regular(100, 0, 700, name=r"gen $e$ $p_T$ $(GeV)$"),
                   lambda objs, mask: abs(objs["genEs"].pt)),
        ],
    ),
    ...
    "genMu_pt": h.Histogram(
        [
            h.Axis(hist.axis.Regular(100, 0, 200, name=r"gen $\mu$ $p_T$ $(GeV)$"),
                   lambda objs, mask: abs(objs["genMus"].pt)),
        ],
    ),
    "genMu_pt_highRange": h.Histogram(
        [
            h.Axis(hist.axis.Regular(100, 0, 700, name=r"gen $\mu$ $p_T$ $(GeV)$"),
                   lambda objs, mask: abs(objs["genMus"].pt)),
        ],
    ),
